{
  ...
  let
      ...
      let
        ...
        wrapModule = module: (
          if builtins.isFunction module then
            args@{pkgs, ...}:
              module (args // {my-packages = packages.${pkgs.system};})
          else module
        );
    ...
    lib = {
      mkNixosConfigurations = extraModules: extraNixpkgs: entrypoints:
        let
          mkConfig = entrypointPath:
            nixpkgs.lib.nixosSystem {
              ...
              modules = [
                ...
                (
                  {pkgs, ...}:
                  {
                    _module.args = let system = pkgs.system; in {
